...
'''
shikomizue: a command-line stegware tool for antivirus evasion. shikomizue allows users to conceal
a executable binary in an arbitrary .png image file. The binary is decomposed into a bitstream and
successive bits are used to overwrite the least significant bits (LSBs) in a reproducible pseudorandom
sequence of pixel RGB values, determined by a seed. Following transport of the steganographic image
file to the target LINUX machine, the binary is extracted and executed in memory, without touching
the file system.
...
def pix_list(image, rand_seed):
    random.seed(rand_seed)
    colors = ['r', 'b', 'g']
    output_list = []
    x_size = image.size[0]
    y_size = image.size[1]
    image_size = x_size * y_size #Number of pixels in image
    totalsize = image_size * 3 #Multiply by 3 to account for each of the rgb channels
...
def concealer(image, payload, seed, output_name):
    ...
    encoding_list = pix_list(im, seed)
